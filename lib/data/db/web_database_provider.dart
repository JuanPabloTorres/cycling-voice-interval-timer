...
/// Web-persistent implementation of [IDatabaseProvider] using SharedPreferences.
/// 
/// Stores data in browser's localStorage, persisting across sessions.
/// Used for web platform where SQLite is not available.
class WebDatabaseProvider implements IDatabaseProvider {
  SharedPreferences? _prefs;
  final Map<String, List<Map<String, dynamic>>> _tables = {};
  bool _isInitialized = false;

  // Storage key prefix
  static const String _keyPrefix = 'ivt_';
  ...
  Future<void> initialize() async {
    ...
    // Load persisted data
    _tables[tableTimerPlans] = _loadTable(tableTimerPlans);
    _tables[tableTimerEvents] = _loadTable(tableTimerEvents);
    _tables[tableRepeatRules] = _loadTable(tableRepeatRules);

    _isInitialized = true;
    print('[WebDatabaseProvider] Initialized with ${_tables[tableTimerPlans]?.length ?? 0} plans from localStorage');
  }

  List<Map<String, dynamic>> _loadTable(String table) {
    final key = '$_keyPrefix$table';
    ...
    try {
      ...
      print('[WebDatabaseProvider] Error loading $table: $e');
      return [];
    }
  }
  ...
  Future<void> _cascadeDelete(List<Object?>? whereArgs) async {
    ...
    await _saveTable(tableTimerEvents);
    
    _tables[tableRepeatRules]?.removeWhere((row) => row['plan_id'] == planId);
    await _saveTable(tableRepeatRules);
  }
